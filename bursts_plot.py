...
def plot(DS, spectrum, ts, extent, subplot_spec, fig, ncols=1, nrows=1, t_scrunch=1., f_scrunch=1., index=None,\
  ...
  ax2.annotate("{} mJy".format(ts[0].max()), xy=(0.1,0.5), xycoords='axes fraction')
  if burst_n: ax2.annotate("Burst {}".format(burst_n), xy=(0.9,0.5), xycoords='axes fraction', ha='right')
